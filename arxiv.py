...
def label_and_score_paper(title, abstract):
    prompt = f"""
        ...
        1. **判断相关性**：  
        - 如果论文**与推荐系统/推荐算法无直接关联**（例如仅涉及通用 NLP、计算机视觉、广告投放、纯搜索排序、数据库索引等），则相关性分数为 **1–3 分**。
        ...
        3. **创新度打分（1–10 分）**：  
        - 10 分：提出全新范式、架构或理论，显著推动领域发展（如首次将 LLM 用于生成式推荐）。
        - 7–9 分：在现有方法上有显著改进或巧妙组合，效果或效率明显提升。
        - 4–6 分：常规改进（如调参、模块替换），无本质创新。
        - 1–3 分：方法陈旧、复现已有工作，或创新点模糊。
        ...
        5. **输出格式**（严格按以下 JSON 格式，不要任何额外解释或 Markdown）：
        {{
